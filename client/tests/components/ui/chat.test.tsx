...
vi.mock('../../../src/components/ui/message-input', () => ({
  MessageInput: ({ value }: { value: string; onChange?: () => void; onSubmit?: () => void }) => (
    <textarea
...
vi.mock('../../../src/components/ui/prompt-suggestions', () => ({
  PromptSuggestions: ({ suggestions, append }: { suggestions: string[]; append: (msg: { role: string; content: string }) => void }) => (
...
describe('Chat', () => {
  ...
  it('debe aplicar className personalizado', () => {
    const { container } = render(<Chat {...defaultProps} className="custom-class" />);
    expect(container.querySelector('.custom-class')).toBeDefined();
  });

  it('debe manejar estado de generación', () => {
    const { rerender } = render(<Chat {...defaultProps} isGenerating={false} />);
    expect(screen.getByRole('textbox')).toBeDefined();
