...
describe('ChatMessage', () => {
  ...
  it('debe renderizar mensaje de asistente', () => {
    render(<ChatMessage {...defaultProps} role="assistant" content="Hi there" />);
  ...
  it('debe aplicar animación scale', () => {
    const { container } = render(<ChatMessage {...defaultProps} animation="scale" />);
    expect(container.firstChild).toBeDefined();
  });

  it('debe aplicar animación fade', () => {
    const { container } = render(<ChatMessage {...defaultProps} animation="fade" />);
    expect(container.firstChild).toBeDefined();
  });

  it('debe aplicar animación none', () => {
    const { container } = render(<ChatMessage {...defaultProps} animation="none" />);
    expect(container.firstChild).toBeDefined();
  });
  ...
  it('debe renderizar mensaje de sistema', () => {
    render(<ChatMessage {...defaultProps} role="system" content="System message" />);
    expect(screen.getByText('System message')).toBeDefined();
  });

  it('debe renderizar múltiples mensajes del mismo usuario', () => {
    const { rerender } = render(<ChatMessage {...defaultProps} content="Message 1" />);
  ...
  it('debe manejar contenido con formato markdown', () => {
    render(<ChatMessage {...defaultProps} role="assistant" content="**bold** text" />);
    expect(screen.getByTestId('markdown')).toBeDefined();
  });

  it('debe renderizar correctamente fechas pasadas', () => {
    const pastDate = new Date('2023-01-01');
    render(<ChatMessage {...defaultProps} createdAt={pastDate} showTimeStamp={true} />);
    expect(screen.getByText('Hello')).toBeDefined();
  });

  it('debe renderizar correctamente fechas futuras', () => {
    const futureDate = new Date('2030-01-01');
    render(<ChatMessage {...defaultProps} createdAt={futureDate} showTimeStamp={true} />);
    expect(screen.getByText('Hello')).toBeDefined();
  });
});
